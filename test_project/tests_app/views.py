...
def user_profile(request):
    ...
    return render(request, 'tests_app/user_profile.html', {'results': user_results})
@login_required
def evaluation_history(request):
    user_results = UserTestresult.objects.filter(user=request.user).order_by('-completed_at')
    return render(request, 'tests_app/evaluation_history.html', {'results': user_results})
@login_required
def compare_results(request):
    user_results = UserTestresult.objects.filter(user=request.user).order_by('-completed_at')
    # Aquí podríamos procesar datos para gráficos comparativos, por ejemplo, usando Matplotlib o Plotly
    return render(request, 'tests_app/compare_results.html', {'results': user_results})